  ...
  return <section className="py-24 px-4 bg-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20">
          {partnerBenefits.map((benefit, index) => <div key={index} className={`bg-white rounded-3xl p-8 text-center hover:scale-105 transition-all duration-300 border-2 ${benefit.borderColor} shadow-lg hover:shadow-xl`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                ...
                <Input name="businessName" value={formData.businessName} onChange={handleInputChange} placeholder="z.B. Blumen Müller" required className="text-lg py-6 border-2 border-gray-300 focus:border-green-500 rounded-xl" />
              </div>
              <div>
                <label className="block text-lg font-medium text-gray-800 mb-3">
                  Ansprechpartner *
                </label>
                <Input name="contactName" value={formData.contactName} onChange={handleInputChange} placeholder="Dein Name" required className="text-lg py-6 border-2 border-gray-300 focus:border-green-500 rounded-xl" />
              </div>
            </div>
            ...
            <div>
              ...
              <Input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="kontakt@deinladen.de" required className="text-lg py-6 border-2 border-gray-300 focus:border-green-500 rounded-xl" />
            </div>
            ...
            <div>
              ...
              <Textarea name="message" value={formData.message} onChange={handleInputChange} placeholder="Erzähl uns kurz von deinem Geschäft..." rows={5} className="text-lg border-2 border-gray-300 focus:border-green-500 resize-none rounded-xl" />
            </div>
            
            <Button type="submit" className="w-full py-8 text-xl bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 transition-all duration-300 shadow-lg hover:shadow-xl rounded-xl text-slate-50">
